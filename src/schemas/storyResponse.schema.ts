...
export const StoryResponseSchema = z
  ...
  `),
    itemState: ItemStateSchema.optional().describe(`
      В случае, если действие игрока привело к получению или потере предметов (например, если игрок купил или нашел новый предмет - добавляй предметы в эту схему). Если предмет был утерян или потрачен, добавляй его в itemLost. Предмет также необходимо считать утраченным и вновь полученным, если он был улучшен (к примеру заточен или модифицирован у кузнеца). Например, если игрок заточил катану у кузнеца - ты должен поместить старую катану в itemsLost, а новую указать в itemsFound  
  `),
  })
  .strict();
