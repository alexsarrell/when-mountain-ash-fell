...
async function main() {
  ...
  await mongoose.connect(
    process.env.MONGODB_URI || "mongodb://localhost:27017/rpg-game",
  );
  ...
  const storyAgent = new StoryAgent(
    chatService,
    process.env.OPENAI_MODEL || "just-ai/openai-proxy/gpt-4.1",
    (process.env.OPENAI_JSON_MODE || "true") === "true",
  );
  const promptAgent = new PromptAgent(
    process.env.OPENAI_PROMPT_AGENT_MODEL || "just-ai/openai-proxy/gpt-4.1",
    chatService,
  );
  
  const imageService = new ImageService(
    process.env.NANO_BANANA_API_KEY || "",
    process.env.NANO_BANANA_URL ||
      "https://caila.io/api/mlpgateway/account/just-ai/model/gemini-nano-banana",
