...
В ходе выполнения проекта была реализована функционирующая текстовая RPG с AI-генерацией контента. Основные достижения:

- **Динамическая генерация нарратива** — система создаёт связное игровое повествование, адаптирующееся к действиям игрока в реальном времени
- **Поддержка D&D-механик** — реализованы броски дайсов (d20, d100) с 3D-визуализацией через Three.js
- **Генерация визуального контента** — динамическое создание спрайтов персонажей и изображений локаций
- **Система инвентаря и экипировки** — полноценное управление предметами со статами и автоматической перегенерацией спрайта при изменении экипировки
- **Real-time взаимодействие** — SSE-стриминг для прогрессивного отображения результатов

### 6.2 Принятые инженерные решения

Ряд архитектурных решений оказал существенное влияние на ход разработки:

| Решение | Обоснование | Результат |
|---------|-------------|-----------|
| Монолитная архитектура | Масштаб учебного проекта не требует распределённой системы | Упрощение разработки и отладки |
| Vanilla JS на фронтенде | Отсутствие необходимости в сложном UI | Ускорение разработки, прямой контроль над DOM |
| Zod-first подход | Единый источник правды для типов | Типобезопасность на всех уровнях приложения |
| Отказ от DI-контейнеров | KISS-принцип | Прозрачный поток зависимостей |
| SSE вместо WebSocket | Односторонний поток данных достаточен | Простота реализации |

### 6.3 Экономическая целесообразность

Анализ эксплуатационных затрат выявил ключевое ограничение для production-развёртывания:

**Структура затрат на одно действие игрока:**
- Генерация нарратива (LLM API): ~0.01-0.05$ за запрос
- Генерация изображения локации: ~0.02-0.05$ за запрос
- Регенерация спрайта персонажа (при изменении экипировки): ~0.02$ за запрос

**Вывод:** текущая стоимость качественных генеративных моделей делает массовую эксплуатацию экономически нецелесообразной. Однако наблюдается устойчивая тенденция к снижению стоимости API-вызовов, что позволяет рассматривать проект как прототип для будущего production-развёртывания.

**Стратегическое решение:** архитектура системы спроектирована с учётом возможности масштабирования при изменении экономических условий рынка генеративных моделей.
...
**Оптимизация затрат:**
- Кэширование типовых генераций (стандартные локации, NPC)
- Оптимизация промптов для снижения количества токенов
- Использование более дешёвых моделей для вспомогательных задач

**Расширение функциональности:**
- Мультиплеерный режим с общим игровым миром
- Система квестов с отслеживанием прогресса
- Боевая система с пошаговыми механиками
- Генерация звукового сопровождения

**Технические улучшения:**
- Миграция на фреймворк (React/Vue) при усложнении UI
- Внедрение очередей задач для асинхронной генерации изображений
- Добавление WebSocket для двустороннего real-time взаимодействия
...
6. Server-Sent Events (MDN) — https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events